...
export function ReasoningWorkspace() {
  ...
  async function handleRun() {
    ...
    try {
      ...
      logEvent(
        "infer",
        `Reasoning inferred ${(data.inferred_facts ?? []).length} facts · ${data.added_edges ?? 0} edges added`,
        {
          facts: data.inferred_facts ?? [],
          rulesFired: data.rules_fired ?? 0,
          addedEdges: data.added_edges ?? 0,
          appliedToGraph: applyToGraph,
        },
      );
