...
interface RawDuplicateItem {
  ...
  entity1?: string | Record<string, unknown>;
  entity2?: string | Record<string, unknown>;
  similarity?: number;
...
function parseDuplicates(raw: RawDuplicateItem[]): DedupPair[] {
  return raw.map((item) => {
    const entityA = item.entity_a ?? item.entity1;
    const entityB = item.entity_b ?? item.entity2;
    return {
      a: {
        id: extractId(entityA),
        label: extractLabel(entityA),
        type: extractType(entityA),
      },
      b: {
        id: extractId(entityB),
        label: extractLabel(entityB),
        type: extractType(entityB),
      },
      score: Number(item.similarity ?? item.similarity_score ?? item.score ?? item.confidence ?? 0),
    };
  });
}
