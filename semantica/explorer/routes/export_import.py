    ...
    if filename.endswith(".json"):
        ...
        for raw_node in raw_nodes:
            if "properties" in raw_node:
                # ContextGraph accepts temporal bounds at the top level, but
                # its portable ``find_node`` payload is metadata-based.  Mirror
                # the bounds into properties so later bounds/export calls do
                # not lose them after ingestion.
                normalized_node = dict(raw_node)
                node_properties = dict(raw_node.get("properties") or {})
                if raw_node.get("valid_from") is not None:
                    node_properties.setdefault("valid_from", raw_node.get("valid_from"))
                if raw_node.get("valid_until") is not None:
                    node_properties.setdefault("valid_until", raw_node.get("valid_until"))
                normalized_node["properties"] = node_properties
