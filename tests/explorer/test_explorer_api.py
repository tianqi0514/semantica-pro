...
class TestImportExport:
    ...
    def test_import_json_preserves_top_level_temporal_bounds(self, client):
        payload = json.dumps(
            {
                "nodes": [
                    {
                        "id": "temporal-import",
                        "type": "event",
                        "properties": {"content": "Imported temporal event"},
                        "valid_from": "2025-01-02T03:04:05+08:00",
        ...
        response = client.post(
            "/api/import",
            files={"file": ("temporal.json", payload, "application/json")},
        )
        assert response.status_code == 200

        node = client.get("/api/graph/node/temporal-import").json()
        assert node["valid_from"] == "2025-01-02T03:04:05+08:00"
        assert node["valid_until"] == "2025-12-31T23:59:59+08:00"

        bounds = client.get("/api/temporal/bounds").json()
...
def test_path_route_adapts_context_graph_payload_without_monkeypatch():
    """The default in-memory Explorer session must be directly traversable."""
    graph = ContextGraph(advanced_analytics=False)
    graph.add_node("decision", node_type="decision", content="Review decision")
    graph.add_node("request", node_type="purchase_request", content="Purchase request")
    graph.add_node("buyer", node_type="person", content="Buyer")
    graph.add_edge("decision", "request", edge_type="reviews")
    graph.add_edge("request", "buyer", edge_type="submittedBy")

    app = create_app(session=GraphSession(graph))
    with TestClient(app) as client:
        response = client.get(
            "/api/graph/path",
            params={"source": "decision", "target": "buyer", "algorithm": "bfs"},
        )

    assert response.status_code == 200
    payload = response.json()
    assert payload["path"] == ["decision", "request", "buyer"]
