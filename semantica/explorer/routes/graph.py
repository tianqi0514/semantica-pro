...
def _as_traversal_graph(graph_source: Any) -> Any:
    """Adapt an Explorer graph payload to the graph interface used by PathFinder.

    ``GraphSession.build_graph_dict`` returns the portable
    ``entities``/``relationships`` representation used by analytics and export.
    PathFinder, however, traverses a NetworkX-like object.  Keeping the adapter
    at the route boundary lets both representations retain their intended use.
    """
    if not isinstance(graph_source, dict):
        return graph_source

    traversal = nx.DiGraph()
    for node in graph_source.get("entities", graph_source.get("nodes", [])) or []:
        node_id = node.get("id") if isinstance(node, dict) else None
        if node_id:
            traversal.add_node(str(node_id), **(node.get("metadata", {}) or {}))

    for edge in graph_source.get("relationships", graph_source.get("edges", [])) or []:
        ...
        attributes.setdefault("weight", float(edge.get("weight", 1.0) or 1.0))
        if edge.get("id") is not None:
            attributes.setdefault("id", edge.get("id"))
        traversal.add_edge(str(source), str(target), **attributes)
...
async def _find_path_impl(
    ...
    traversal_graph = _as_traversal_graph(graph_dict)
    path_fn = (
    ...
    try:
        result = await asyncio.to_thread(path_fn, traversal_graph, source, target, directed=directed)
        ...
        if path_finder is not None and hop_count > 0:
            try:
                k_paths = await asyncio.to_thread(
                    ...
                    traversal_graph, source, target, hop_count + 2, directed=directed
                )
