...
import { I18nDomBridge, LanguageSwitcher } from './I18nDomBridge';
...
const DocumentExtractionWorkspace = lazy(() => import('./workspaces/DocumentExtractionWorkspace/DocumentExtractionWorkspace').then((module) => ({ default: module.DocumentExtractionWorkspace })));

type WorkspaceId = 'welcome' | 'explore' | 'analyze' | 'decisions' | 'documents' | 'enrich' | 'manage' | 'ontology-hub';
...
const navItems: NavItem[] = [
  ...
  { id: 'documents', label: 'Documents', hint: 'Extract entities and relationships from documents', icon: ScanText },
...
function WelcomeScreen({
  ...
  const secondaryLaunchers: LandingAction[] = [
    ...
    {
      label: 'Documents',
      description: 'Extract candidate knowledge',
      icon: ScanText,
...
export default function App() {
  ...
  const [documentView, setDocumentView] = useState<DocumentView>('extract');
  ...
  const renderWorkspace = () => {
    ...
    if (activeWorkspace === 'documents') {
      return (
        <WorkspaceShell
          title="文档知识抽取"
          subtitle="把业务文档转换为带来源证据的候选实体与关系。"
        ...
        >
          <ErrorBoundary key={`documents-${documentView}`}>
            <Suspense fallback={<WorkspaceFallback />}>
              <DocumentExtractionWorkspace view={documentView} />
            </Suspense>
          </ErrorBoundary>
        </WorkspaceShell>
      );
